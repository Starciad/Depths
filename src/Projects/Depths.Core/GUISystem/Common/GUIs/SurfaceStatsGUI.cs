...
{
    internal sealed class SurfaceStatsGUI : GUI
    {
        ...
        private readonly GUIImageElement[] oreIconElements;
        private readonly GUITextElement moneyTextElement;
        private readonly GUITextElement oreCountingTextElement;
        ...
        private readonly sbyte yFinalPosition = -ScreenConstants.GAME_HEIGHT;
        ...
        internal SurfaceStatsGUI(string identifier, AssetDatabase assetDatabase, TextManager textManager, GUIManager guiManager, GameInformation gameInformation) : base(identifier)
        {
            ...
            this.oreIconElements = new GUIImageElement[5];

            for (byte i = 0; i < this.oreIconElements.Length; i++)
            {
                GUIImageElement oreIconElement = new()
                {
                    IsVisible = false,
                    Position = new(11 + (i * (SpriteConstants.ORE_ICON_SIZE + 2)), 16)
                };
        ...
        private void UpdateScoreAnimation()
        {
            ...
            if (this.gameInformation.PlayerEntity.CollectedMinerals.TryDequeue(out Ore ore))
            {
                AudioEngine.Play("sound_blip_11");
        ...
        {
            foreach (GUIImageElement oreIconElement in this.oreIconElements)
            {
